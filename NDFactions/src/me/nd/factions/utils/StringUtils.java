...
public class StringUtils {
	...
	public static String deformatColors(String textToDeFormat) {
		...
		while (matcher.find()) {
			...
			textToDeFormat = textToDeFormat.replaceFirst(Pattern.quote(color),
	...
	/**
	 * Quebra uma {@code String} várias vezes para criar linhas com o tamanho máximo
	 * definido.<br/>
	 * <b>Observação:</b> Esse método é uma variação do
	 * {@link StringUtils#split(String, int, boolean)} com o parâmetro
	 * {@code ignoreCompleteWords} definido como {@code false}.
	 *
	...
	public static String capitalise(String toCapitalise) {
		...
		for (int index = 0; index < splittedString.length; index++) {
			...
			result.append(append.substring(0, 1).toUpperCase() + append.substring(1).toLowerCase()
					+ (index + 1 == splittedString.length ? "" : " "));
		}
	 ...
	 * @param toSplit               String para quebrar.
	 * @param length                Tamanho máximo das linhas.
	 * @param ignoreIncompleteWords Se irá ignorar a quebra de palavras ou não (caso
	 *                              esteja desativado e for quebrar uma palavra, ela
	 *                              irá ser removida da linha atual e adicionar na
	 *                              próxima).
	 * @return Resultado da separação.
	 */
