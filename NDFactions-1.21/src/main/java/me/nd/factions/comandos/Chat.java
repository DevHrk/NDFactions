		...
		if (sender instanceof Player) {
			if (cmd.getName().equals(".")) {
				...
				for (Player all : player.getFaction().getAllOnline()) {
					all.sendMessage("§a"+player.getCargoSimbolo()+"§f"+player.getNome()+": §b"+msg);
				}
			}
			if (cmd.getName().equals("a")) {
				...
				for (Player all : online) {
					all.sendMessage("§e[§a"+player.getCargoSimbolo()+player.getFaction().getNome()+"§e] §f"+player.getNome()+": §3"+msg);
				}
			}
		}
		return false;
