...
public class Methods extends MySQL {
    ...
    public static void updateFaction(NDFaction faction) throws SQLException {
        String query = "REPLACE INTO `NDFactions`(`nome`, `base`, `banco`, `terras`, `aliados`, `capitoes`, `membros`, `recrutas`, `lider`, `tag`, `inimigos`, `permissoes`, `permissoes_relacoes`, `permissoes_membros`, `generators`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try (PreparedStatement stmt = getConnection().prepareStatement(query)) {
