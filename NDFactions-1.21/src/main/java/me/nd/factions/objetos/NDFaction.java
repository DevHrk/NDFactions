...
public class NDFaction {
    ...
    public static class RelationLog {
        ...
        private final String action; // e.g., "set", "removed"
        private final long timestamp;
    ...
    private void setupPermissoes() {
        ...
        // Initialize per-member permissions for existing members
        for (String member : getAllMembers()) {
            Map<String, Boolean> memberPerms = new HashMap<>();
            memberPerms.put("abrir_bau", false);
            memberPerms.put("abrir_porta", false);
            memberPerms.put("apertar_botao", false);
            memberPerms.put("teleportar", false);
            memberPerms.put("colocar_bloco", false);
            memberPerms.put("quebrar_bloco", false);
    ...
    public boolean hasPermissaoMembro(String playerName, String permissao) {
        Map<String, Boolean> memberPerms = permissoesMembros.getOrDefault(playerName, new HashMap<>());
        return memberPerms.getOrDefault(permissao, false);
    }
    ...
    public double getTotalMoneyEmSpawners() {
        ...
        double total = 0;
        // Combine placed and stored spawners to calculate total quantity per type
        ...
        // Calculate value for each spawner type
    ...
    public void addPlacedGenerator(EntityType type, int amount, String playerName) {
        ...
        cachedTotalMoney = -1; // Invalidate cache
        try {
            save();
        } catch (Exception e) {
    ...
    public boolean ownsTerritory(Terra terra) {
        ...
        // Verifica se o chunk está nos territórios permanentes ou temporários
        for (Terra t : terras) {
            if (t.equals(terra)) {
                return true;
            }
            ...
            for (int z = cz; z < cz + 16; z++) {
                for (int y = world.getMinHeight(); y < world.getMaxHeight(); y++) {
                    ...
                    if (block.getType() == Material.SPAWNER) {
                        ...
                        if (type != null && FactionGenerators.GENERATOR_NAMES.containsKey(type)) {
                            addStoredGenerator(type, 1, playerName);
                            removePlacedGenerator(type, 1, null);
    ...
    public boolean addMember(NDPlayer player) {
        ...
        // Initialize permissions for the new member
        Map<String, Boolean> memberPerms = new HashMap<>();
        memberPerms.put("abrir_bau", false);
        memberPerms.put("abrir_porta", false);
        memberPerms.put("apertar_botao", false);
                    ...
                    for (int y = world.getMinHeight(); y < world.getMaxHeight(); y++) {
                        ...
                        if (block.getType() == Material.SPAWNER) {
                            ...
                            if (type != null && FactionGenerators.GENERATOR_NAMES.containsKey(type)) {
                                ...
                                Bukkit.getLogger().warning("Spawner sem tipo definido ou não suportado encontrado em " + x + "," + y + "," + z + " (mundo: " + world.getName() + ")");
                            }
                        }
                    }
                }
    ...
    public void syncPlacedGenerators() {
                ...
                for (int z = cz; z < cz + 16; z++) {
                    for (int y = world.getMinHeight(); y < world.getMaxHeight(); y++) {
                        ...
                        if (block.getType() == Material.SPAWNER) {
                            ...
                            EntityType type = spawner.getSpawnedType();
                            if (type != null && FactionGenerators.GENERATOR_NAMES.containsKey(type)) {
                                worldSpawners.merge(type, 1, Integer::sum);
                            }
                        }
                    }
                }
        ...
        try {
            ...
            Bukkit.getLogger().severe("Erro ao salvar placedGenerators sincronizado para " + nome + ": " + e.getMessage());
        }
    }
    ...
    public void disband() {
        ...
        for (NDPlayer player : allPlayers) {
            if (player != null) {
                ...
                // Check if the player is not the leader before sending the message
